...
export function TriageMode() {
  ...
  return (
    <div className="absolute inset-0 z-40 bg-[#0b0a12]/97 backdrop-blur flex flex-col items-center justify-center">
      <div className="absolute top-5 left-6 text-[14px] text-zinc-500">
        ⚡ Speed triage —{" "}
        <span className="text-zinc-300">{queue.length} left</span>
      </div>
      ...
      {!current ? (
        <div className="text-center animate-pop-in">
          ...
          <div className="text-zinc-400 mt-1 text-sm">
            {triageStreak} emails triaged. Superhuman who?
          </div>
        </div>
      ) : (
        <div key={current.id} className="w-[640px] animate-pop-in">
          <div className="bg-[#161325] border border-white/10 rounded-2xl p-7 shadow-2xl">
          ...
          <div className="flex justify-center gap-3 mt-6 text-[14px]">
            ...
            {current.timeIntent && (
              <Key k="S" label="Schedule + invite" accent />
            )}
