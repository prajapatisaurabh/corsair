...
export function TriageMode() {
  ...
  return (
    <div className="absolute inset-0 z-40 bg-[#0a0a0f]/97 backdrop-blur flex flex-col items-center justify-center">
      <div className="absolute top-5 left-6 text-[12px] text-zinc-500">
        ⚡ Speed triage — <span className="text-zinc-300">{queue.length} left</span>
      </div>
      <div className="absolute top-5 right-6 flex items-center gap-4 text-[12px]">
      ...
      {!current ? (
        ...
        <div key={current.id} className="w-[640px] animate-pop-in">
          <div className="bg-[#13131c] border border-white/10 rounded-2xl p-7 shadow-2xl">
            ...
            <p className="text-[13.5px] text-zinc-300 leading-relaxed line-clamp-4 whitespace-pre-wrap">
              {current.body}
            </p>
            {current.timeIntent && (
              <div className="mt-4 text-[12px] text-amber-300 bg-amber-500/10 border border-amber-500/25 rounded-md px-3 py-2 inline-block">
                ⏰ mentions {current.timeIntent.phrase}
              </div>
            )}
          </div>

          <div className="flex justify-center gap-3 mt-6 text-[12.5px]">
...
function Key({ k, label, accent }: { k: string; label: string; accent?: boolean }) {
  return (
    ...
    >
      <kbd className="!text-[12px]">{k}</kbd> {label}
    </div>
  );
}
