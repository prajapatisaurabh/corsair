...
export async function PATCH(req: NextRequest) {
  ...
  if (isLive()) {
    ...
    try {
      ...
      if (patch.archived) {
        await corsair.gmail.messages.modify({ id, removeLabelIds: ["INBOX"] });
      }
      if (patch.unread === false) {
        await corsair.gmail.messages.modify({ id, removeLabelIds: ["UNREAD"] });
      }
    } catch (err) {
...
export async function POST(req: NextRequest) {
  ...
  if (isLive()) {
    try {
      ...
      await corsair.gmail.messages.send({ raw });
    } catch (err) {
